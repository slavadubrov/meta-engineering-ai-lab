...
| --- | ---: | ---: | ---: |
| Downstream success (fraction) | 65.0% | 95.0% | 100.0% |
| Useful-write precision (fraction) | 100.0% | 100.0% | 100.0% |
| Useful-write recall (fraction) | 97.4% | 97.4% | 100.0% |
| Unnecessary-write rate (fraction) | 0.0% | 0.0% | 0.0% |
| Relevant-memory recall (fraction) | 90.5% | 95.2% | 100.0% |
| Distracting-memory rate (fraction) | 48.6% | 4.8% | 4.5% |
| Stale/future fact rate (fraction) | 14.8% | 0.0% | 0.0% |
| Update correctness (fraction) | 100.0% | 100.0% | 100.0% |
| Mean context words (words) | 4.11 | 2.33 | 2.44 |
| Mean stored records (records) | 1.70 | 1.70 | 1.65 |
| Mean stored bytes (bytes) | 416.35 | 416.35 | 403.05 |
| Local p50 latency (ms) | 0.1315 | 0.0609 | 0.0558 |
| Local p95 latency (ms) | 0.8612 | 0.2428 | 0.2090 |
...
Enabling entity filtering and time-aware retrieval will remove cross-entity distractors and exclude records outside the requested validity interval, improving complete-scenario success.
...
Lowering the confidence threshold to 0.6 will retain the demonstrated useful low-confidence fact, while enabling deduplication will prevent repeated identical confirmations from creating extra active-history records without changing their answer.

Patch: `{"deduplicate": true, "min_confidence": 0.6}`. Parent: `agent-01`.
...
uv run --frozen python -m lab verify artifacts/agent-study-02/campaign-03/iteration-02/evaluation
```
