...
| Local p50 latency (ms) | 0.1249 | 0.0617 | 0.0580 |
| Local p95 latency (ms) | 0.5047 | 0.2474 | 0.3405 |
...
Applying entity and validity-date filtering will remove cross-entity and temporally invalid candidates before ranking, fixing the demonstrated retrieval failures; deduplication is deferred because the primary errors are filtering errors and only two settings may change.
...
Enabling deduplication will reuse identical active values, preserving correct retrieval while reducing redundant stored records.
...
uv run --frozen python -m lab verify artifacts/agent-study-02/campaign-02/iteration-02/evaluation
```
