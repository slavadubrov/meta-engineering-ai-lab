...
| Metric | Baseline | Agent proposal 1 | Agent proposal 3 | Agent proposal 4 |
| --- | ---: | ---: | ---: | ---: |
...
| Mean stored bytes (bytes) | 416.35 | 416.35 | 390.45 | 403.05 |
| Local p50 latency (ms) | 0.0468 | 0.0458 | 0.0556 | 0.0533 |
| Local p95 latency (ms) | 0.2075 | 0.3019 | 0.2685 | 0.2031 |
...
Lowering min_confidence from 0.7 to 0.6 will admit the demonstrated useful Portuguese fact while continuing to reject the supplied 0.4 and 0.3 low-confidence facts, improving complete-scenario success without changing retrieval isolation or deduplication behavior.

Patch: `{"min_confidence": 0.6}`. Parent: `agent-03`.
...
uv run --frozen python -m lab verify artifacts/agent-study-01/campaign-01/iteration-04/evaluation
```
