...
| Metric | Baseline | Agent proposal 1 | Agent proposal 2 |
| --- | ---: | ---: | ---: |
...
| Local p50 latency (ms) | 0.0423 | 0.0593 | 0.0535 |
| Local p95 latency (ms) | 0.2025 | 0.2293 | 0.2062 |
...
Enabling entity and as-of-date filtering will prevent cross-entity distractors and exclude records outside the requested validity interval, improving complete-scenario task success without changing admission or ownership behavior.
...
Enabling deduplication will reuse identical active values, reducing stored records and duplicate retrieval distraction while preserving answers and hard-constraint behavior.
...
uv run --frozen python -m lab verify artifacts/agent-study-01/campaign-03/iteration-02/evaluation
```
