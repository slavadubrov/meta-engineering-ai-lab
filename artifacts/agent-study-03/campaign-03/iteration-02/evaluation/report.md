...
| Mean stored bytes (bytes) | 532.55 | 532.55 | 499.65 |
| Local p50 latency (ms) | 0.1163 | 0.0833 | 0.0683 |
| Local p95 latency (ms) | 0.8706 | 0.3543 | 0.2410 |
...
Applying entity and temporal filters before ranking will remove cross-entity distractors and records outside the requested as-of interval, improving complete-scenario retrieval correctness without changing writes or hard-constraint behavior.
...
Enabling deduplicate will reuse an identical active value confirmed at a later effective date, reducing redundant records while preserving retrieval correctness and hard-constraint behavior.
...
uv run --frozen python -m lab verify artifacts/agent-study-03/campaign-03/iteration-02/evaluation
```
